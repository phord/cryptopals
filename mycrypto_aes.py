...
def matrix_InvSubst(vec):
    return [ [rsbox[x] for x in row] for row in vec]
...
def expandKey(key):
    ...
    while (len(expandedKey) < expandedKeySize):
        ...
        if len(expandedKey) % key_size == 0:
            rconIteration += 1
            t = core(t, rconIteration)

        # For 256-bit keys, we add an extra sbox to the calculation
        if key_size == 32 and (len(expandedKey) % 32) == 16:
            t = [ getSBoxValue(x) for x in t ]

        # We XOR t with the four-byte block 16,24,32 bytes before the new expanded key.
        # This becomes the next four bytes in the expanded key.
        expandedKey += enc_xor(expandedKey[-key_size:-key_size+4], t)
...
UnMixMatrix = aes_bin2matrix([14,9,13,11, 11,14,9,13, 13,11,14,9, 9,13,11,14])[0]
def GF(x,y):
    if y==1: return x
    return GFlookup[y][x]
...
def unpad(vec):
    padlen = vec[-1]
    if padlen > len(vec):
        raise Exception("Pad length too long")
    if vec[-padlen:] != [padlen]*padlen:
...
def test():
    ...
    verify = [ 0x29, 0xC3, 0x50, 0x5F, 0x57, 0x14, 0x20, 0xF6, 0x40, 0x22, 0x99, 0xB3, 0x1A, 0x02, 0xD7, 0x3A ]

    vec = aes_encrypt(plaintext, password)
    print verify == vec, ' '.join(blockSplit(bin2hex(vec),2))

    dvec = aes_decrypt(vec, password)
    print plaintext == dvec, bin2str(dvec)
